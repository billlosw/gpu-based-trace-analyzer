...
|-------|-----------|--------|-----------|
| CG Class B | 64 | 2.5M | Yes - exact match on 7/8 |
| CG Class C | 64 | 2.5M | Yes - exact match on 7/8 |
...
| late_receiver | Over-counts 3-4x | Expected: we count all, Scalasca only counts blocking |
...
### CG Class C Detailed Results

| Metric | Scalasca | GPU Analyzer | Match |
|--------|----------|--------------|-------|
| late_sender Count | 396,922 | 396,922 | EXACT |
| late_sender Sum | 14.2017681416 | 14.2017681417 | ~1 ps |
| late_sender Max | 0.0561983636 | 0.0561983636 | EXACT |
...
```bash
# Run GPU analyzer
srun --mpi=pmix -n 8 --gres=gpu:4090:1 \
